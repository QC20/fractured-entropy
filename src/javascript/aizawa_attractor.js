// Aizawa Attractor with RK4 Integration and Adaptive Performance
// ============================================================
...
function updateParticlesRK4() {
    for (var i = 0; i < currentDots; i++) {
        ...
        // k1 = f(t, y)
        var derivs1 = getAizawaDerivatives(x[i], y[i], z[i]);
        k1x[i] = derivs1.dx;
        k1y[i] = derivs1.dy;
        k1z[i] = derivs1.dz;
        
        // k2 = f(t + dt/2, y + k1*dt/2)
        var derivs2 = getAizawaDerivatives(
            x[i] + k1x[i] * dt * 0.5,
            y[i] + k1y[i] * dt * 0.5,
            z[i] + k1z[i] * dt * 0.5
        );
        ...
        var derivs3 = getAizawaDerivatives(
            x[i] + k2x[i] * dt * 0.5,
            y[i] + k2y[i] * dt * 0.5,
            z[i] + k2z[i] * dt * 0.5
        );
        ...
        // k4 = f(t + dt, y + k3*dt)
        var derivs4 = getAizawaDerivatives(
            x[i] + k3x[i] * dt,
            y[i] + k3y[i] * dt,
            z[i] + k3z[i] * dt
        );
...
function getAizawaDerivatives(px, py, pz) {
    return {
        dx: (pz - b) * px - d * py,
        dy: d * px + (pz - b) * py,
        dz: c + a * pz - (pz * pz * pz) / 3 - (px * px + py * py) * (1 + e * pz) + f * pz * (px * px * px)
    };
}
