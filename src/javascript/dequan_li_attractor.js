// Dequan Li Chaotic Attractor with Adaptive Performance
// Mathematical system: dx/dt = a(y-x) + dyz, dy/dt = kx + fy - xz, dz/dt = gxy - hz
...
// Dequan Li specific parameters
var a = 40;    // system parameter a
var d = 1.833; // system parameter d
var k = 20;    // system parameter k
var f = 5;     // system parameter f
var g = 1;     // system parameter g
var h = 9;     // system parameter h
...
function updateParticlesRK4() {
    for (var i = 0; i < currentDots; i++) {
        ...
        // k1 = f(t, y)
        var derivs1 = getDequanLiDerivatives(x[i], y[i], z[i]);
        k1x[i] = derivs1.dx;
        k1y[i] = derivs1.dy;
        k1z[i] = derivs1.dz;
        
        // k2 = f(t + dt/2, y + k1*dt/2)
        var derivs2 = getDequanLiDerivatives(
            x[i] + k1x[i] * dt * 0.5,
            y[i] + k1y[i] * dt * 0.5,
            z[i] + k1z[i] * dt * 0.5
        );
        ...
        var derivs3 = getDequanLiDerivatives(
            x[i] + k2x[i] * dt * 0.5,
            y[i] + k2y[i] * dt * 0.5,
            z[i] + k2z[i] * dt * 0.5
        );
        ...
        // k4 = f(t + dt, y + k3*dt)
        var derivs4 = getDequanLiDerivatives(
            x[i] + k3x[i] * dt,
            y[i] + k3y[i] * dt,
            z[i] + k3z[i] * dt
        );
...
function getDequanLiDerivatives(px, py, pz) {
    return {
        dx: a * (py - px) + d * py * pz,
        dy: k * px + f * py - px * pz,
        dz: g * px * py - h * pz
