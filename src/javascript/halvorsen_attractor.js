// Halvorsen Chaotic Attractor Implementation
// Parameters: a=1.89, b=1.4, c=0.3, d=2.7
...
function updateParticlesRK4() {
    for (var i = 0; i < currentDots; i++) {
        ...
        // k1 = f(t, y)
        var derivs1 = getHalvorsenDerivatives(x[i], y[i], z[i]);
        k1x[i] = derivs1.dx;
        k1y[i] = derivs1.dy;
        k1z[i] = derivs1.dz;
        
        // k2 = f(t + dt/2, y + k1*dt/2)
        var derivs2 = getHalvorsenDerivatives(
            x[i] + k1x[i] * dt * 0.5,
            y[i] + k1y[i] * dt * 0.5,
            z[i] + k1z[i] * dt * 0.5
        );
        ...
        var derivs3 = getHalvorsenDerivatives(
            x[i] + k2x[i] * dt * 0.5,
            y[i] + k2y[i] * dt * 0.5,
            z[i] + k2z[i] * dt * 0.5
        );
        ...
        // k4 = f(t + dt, y + k3*dt)
        var derivs4 = getHalvorsenDerivatives(
            x[i] + k3x[i] * dt,
            y[i] + k3y[i] * dt,
            z[i] + k3z[i] * dt
        );
...
function getHalvorsenDerivatives(px, py, pz) {
    // Halvorsen attractor equations:
    // dx/dt = -a*x - b*y - c*z - d*y^2
    // dy/dt = -a*y - b*z - c*x - d*z^2
    // dz/dt = -a*z - b*x - c*y - d*x^2
    return {
        dx: -a*px - b*py - c*pz - d*py*py,
        dy: -a*py - b*pz - c*px - d*pz*pz,
        dz: -a*pz - b*px - c*py - d*px*px
    };
}
