// Nose-Hoover Thermostat Attractor with Adaptive Performance
// A chaotic system that models thermostated particle dynamics
...
var a = 1.4;    // System parameter
var b = 0.3;    // System parameter
var c = 1.0;    // System parameter
...
function draw() {
    ...
    // Update particles using Nose-Hoover equations
    for (var i = 0; i < currentDots; i++) {
        px[i] = x[i];
        py[i] = y[i];
        pz[i] = z[i];
        
        // Nose-Hoover thermostat equations
        var dx = y[i];
        var dy = -x[i] + a * y[i] * z[i];
        var dz = b - c * z[i] - x[i] * y[i];
