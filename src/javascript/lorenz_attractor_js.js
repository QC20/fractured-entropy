// Lorenz Attractor with Runge-Kutta 4th Order Integration and Adaptive Performance
// Parameters: sigma=10, rho=28, beta=8/3 (classic Lorenz system)
...
function setup() {
    ...
    // Initialize particles with small random positions around origin
...
function updateParticlesRK4() {
    for (var i = 0; i < currentDots; i++) {
        ...
        // k1 = f(t, y)
        var derivs1 = getLorenzDerivatives(x[i], y[i], z[i]);
        k1x[i] = derivs1.dx;
        k1y[i] = derivs1.dy;
        k1z[i] = derivs1.dz;
        
        // k2 = f(t + dt/2, y + k1*dt/2)
        var derivs2 = getLorenzDerivatives(
            x[i] + k1x[i] * dt * 0.5,
            y[i] + k1y[i] * dt * 0.5,
            z[i] + k1z[i] * dt * 0.5
        );
        ...
        var derivs3 = getLorenzDerivatives(
            x[i] + k2x[i] * dt * 0.5,
            y[i] + k2y[i] * dt * 0.5,
            z[i] + k2z[i] * dt * 0.5
        );
        ...
        // k4 = f(t + dt, y + k3*dt)
        var derivs4 = getLorenzDerivatives(
            x[i] + k3x[i] * dt,
            y[i] + k3y[i] * dt,
            z[i] + k3z[i] * dt
        );
...
function getLorenzDerivatives(px, py, pz) {
    return {
        dx: sigma * (py - px),
        dy: px * (rho - pz) - py,
        dz: px * py - beta * pz
