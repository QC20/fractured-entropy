// Rabinovich-Fabrikant Chaotic Attractor System
// A 3D chaotic system with adaptive performance optimization
...
function draw() {
    ...
    // Update particles using Rabinovich-Fabrikant equations
    for (var i = 0; i < currentDots; i++) {
        ...
        var dx = y[i] * (z[i] - 1 + x[i] * x[i]) + gamma * x[i];
        var dy = x[i] * (3 * z[i] + 1 - x[i] * x[i]) + gamma * y[i];
        var dz = -2 * z[i] * (alpha + x[i] * y[i]);
